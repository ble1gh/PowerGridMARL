...
class PowerGridworldGraphTask(Task):
    ...
    EVOVERNIGHT13NODE = (
        None  # Loaded automatically from conf/task/PowerGridworldGraph/evovernight13node
    )
    EVOVERNIGHT13NODE_SIMPLE = (
        None  # Loaded automatically from conf/task/PowerGridworldGraph/evovernight13node_simple
    )
    EVOVERNIGHT13NODE_VARIABLE = (
        None  # Loaded automatically from conf/task/PowerGridworldGraph/evovernight13node_variable
    )
...
class PowerGridworldGraphClass(TaskClass):
    def get_env_fun(
        ...
        print(
            f"  - {len(Storage_busses) * Storage_per_node} Storage agents ({Storage_per_node} per node)"
        )
