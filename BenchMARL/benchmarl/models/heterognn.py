...
class HeteroGNN(Model):
    ...
    def __init__(
        ...
        edge_features_key: Optional[str] = None,
        agent_node_index_key: Optional[str] = None,
        node_types: Optional[List[str]] = None,
        agent_groups: Optional[List[str]] = None,
        node_features_keys: Optional[Dict[str, str]] = None,
        edge_types: Optional[List[tuple]] = None,
        node_features_dims: Optional[Dict[str, int]] = None,
        edge_features_dims: Optional[dict] = None,
        grid_edge_keys: Optional[Dict[str, str]] = None,
        ...
        **kwargs,
        ...
        self.agent_groups = agent_groups if agent_groups is not None else [kwargs.get("agent_group")]
        # Ensure agent_groups are in node_types
        default_node_types = self.agent_groups + ["grid_node"] if (edge_features_key or grid_edge_keys) else self.agent_groups
        ...
        if self.cat_observations_to_output:
            ...
            if obs_dim >= self.output_features:
                 raise ValueError(f"Output spec size ({self.output_features}) is smaller than observation size ({obs_dim}) "
        ...
        for i in range(self.num_layers):
            ...
            for src, rel, dst in self.edge_types:
                ...
                if edge_dim is None and rel == "interaction" and (self.position_key or self.velocity_key):
    ...
    def _perform_checks(self):
        ...
        if self.topology == "adjacency" and self.edge_features_key is None and self.grid_edge_keys is None:
             raise ValueError("If topology is adjacency, edge_features_key or grid_edge_keys must be provided")
        if (
        ...
        if not self.input_has_agent_dim:
            pass # Relax this check for multi-group scenarios?
            # raise ValueError(
        ...
        # if "agent" not in self.node_types:
        #      raise ValueError("HeteroGNN must include 'agent' in node_types")
             
        for edge in self.edge_types:
            if len(edge) != 3:
                raise ValueError(f"Each element in edge_types must be a tuple of length 3 (src, rel, dst), got {edge}")
            src, rel, dst = edge
    ...
    def _forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        for group in self.agent_groups:
            ...
            has_explicit_features = (
                ...
                and group in self.node_features_keys
            )
            ...
            if (not self.exclude_observations_from_node_features) or self.cat_observations_to_output:
                observations = [
                    ...
                    and _unravel_key_to_tuple(in_key)[-1] not in (self.position_key, self.velocity_key)
                ]
                ...
                if node_type in x_dict and expected_dim > 0:
                    ...
                    if ref is not None and n_cnt > 0:
                         ...
                         x_dict[node_type] = torch.zeros(
                             *batch_shape, n_cnt, expected_dim, 
                             device=ref.device, dtype=torch.float
                         )
        ...
        for k in keys:
            ...
            if k_tuple[-1] == key:
                if group is None or group in k_tuple:
                     if not "next" == k_tuple[0]:
                        return k
        # If group is None, try looser match
        if group is None:
             for k in keys:
                 k_tuple = _unravel_key_to_tuple(k)
                 if k_tuple[-1] == key and not "next" == k_tuple[0]:
                     return k
        return None
...
def _tensordict_to_hetero_data(
    ...
    for node_type in node_types:
        if node_type in x_dict:
            ...
            batch_data[node_type].ptr = torch.arange(0, (batch_size + 1) * n_agents, n_agents, device=device)
            ...
            if n_grid_nodes > 0:
                 ...
                 batch_data[node_type].ptr = torch.arange(0, (batch_size + 1) * count, count, device=device)
    ...
    for src, rel, dst in edge_types:
        ...
        # 3. Radius Graph
             ...
             if src == dst:
                 ...
                 row, col = torch_geometric.nn.pool.radius(x=pos_src, y=pos_dst, r=edge_radius, batch_x=batch_src, batch_y=batch_dst)
