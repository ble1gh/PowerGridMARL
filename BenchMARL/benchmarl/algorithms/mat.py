...
class MultiAgentTransformer(nn.Module):
    def __init__(self, n_agent, obs_dim, action_dim, n_block, n_embd, n_head, action_type='continuous', device='cpu'):
        ...
        self.encoder_blocks = nn.ModuleList([EncodeBlock(n_embd, n_head, n_agent) for _ in range(n_block)])
        ...
        self.decoder_blocks = nn.ModuleList([DecodeBlock(n_embd, n_head, n_agent) for _ in range(n_block)])
...
class MAT(Algorithm):
    ...
    def _get_policy_for_loss(
        ...
        actor_module = TensorDictModule(
            ...
            in_keys=[(group, "observation"), (group, "action")], # Takes ACTION as input (Teacher Forcing)
