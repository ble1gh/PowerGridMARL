...
class MultiAgentListInterfaceEnv(gym.Env):
    ...
    def __init__(self, multi_agent_env_cls, env_config):
        ...
        for k, v in self.nested_sequence.items():
            ...
            agent_obs_space = gym.spaces.Box(
                shape=(agent_obs_len,), low=-1.0, high=1.0, dtype=np.float64
            )
            agent_action_space = gym.spaces.Box(
                shape=(agent_act_len,), low=-1.0, high=1.0, dtype=np.float64
            )
    ...
    def get_nested_sequence(agent_config):
        ...
        for item in agent_config:
            nested_agents_components_sequence[item["name"]] = [
                x["name"] for x in item["config"]["components"]
            ]
    ...
    def step(self, action):
        ...
        reward_list_interface = [reward[k] for k in self.nested_sequence.keys()]
