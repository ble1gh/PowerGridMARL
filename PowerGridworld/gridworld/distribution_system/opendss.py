...
class OpenDSSSolver(PowerFlowSolver):
    def __init__(
        ...
        **kwargs,
        ...
        if len(self.annual_hourly_load_profile) != 8760:
            print(
                "Warning: The provided load shape file is not annual hourly ",
    ...
    def get_bus_connectivity(self) -> tuple[list[str], dict[str, np.ndarray]]:
        ...
        return all_nodes, {"line": adj_line, "transformer": adj_xfmr, "switch": adj_switch}
    ...
    def calculate_power_flow(
        ...
        current_step_load = (
            self.actual_load_coefficients[:, np.newaxis]
            * self.base_load
    ...
    def get_bus_voltage_by_name(self, bus_name: any) -> float | list[float]:
    ...
    def get_nodal_load_forecast(self, current_time: any = None) -> dict[str, np.ndarray]:
        ...
        if self.forecast_load_coefficients is not None:
            ...
            if current_time is None:
                raise ValueError(
                    "current_time must be provided when forecast "
        ...
        current_step_loads = (
            coeffs[:, np.newaxis] * self.base_load * self.system_load_rescale_factor
        )
