...
ZONE_TEMP_BOUNDS = (16.0, 40.0)
# min/max temp violation before clipping
ZONE_UPPER_TEMP_VIOL_BOUNDS = (-10.0, 10.0)
ZONE_LOWER_TEMP_VIOL_BOUNDS = (-10.0, 10.0)
# lower end of comfort temp goes between (a, b) throughout the day
COMFORT_TEMP_LOWER_BOUNDS = (20.0, 23.0)
# upper end of comfort temp goes between (a, b) throughout the day
COMFORT_TEMP_UPPER_BOUNDS = (23.0, 26.0)
# min/max outdoor temp
OUTDOOR_TEMP_BOUNDS = (0.0, 56.0)
...
def _get_obs_dim(num_zones: int, config: list | dict) -> int:
...
def make_obs_space(num_zones: int, config: dict) -> tuple[gym.spaces.Box, list]:
    ...
    for key in config:
        assert key in DEFAULT_OBS_CONFIG, f"invalid key {key}"
