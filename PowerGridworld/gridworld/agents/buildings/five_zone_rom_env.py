...
MIN_FLOW_RATE = [0.22, 0.22, 0.22, 0.22, 0.32]  # Max flow rate for each individual zone
...
DEFAULT_COMFORT_BOUNDS = (22.0, 28.0)  # Temps between these values are considered "comfortable"
...
class FiveZoneROMEnv(ComponentEnv):
    ...
    def __init__(
        ...
        comfort_bounds: tuple | np.ndarray | pd.DataFrame = None,
        ...
        **kwargs,
        ...
        if zone_temp_init is not None:
            ...
            self.zone_temp_init = 27.0 * np.ones(self.num_zones, dtype=np.float64)
        ...
        self.comfort_bounds = (
            comfort_bounds if comfort_bounds is not None else DEFAULT_COMFORT_BOUNDS
        )
        ...
        self._action_space = gym.spaces.Box(low=self.act_low, high=self.act_high, dtype=np.float64)
        ...
        self._observation_space, self._obs_labels = make_obs_space(self.num_zones, self.obs_config)
    ...
    def reset(self, **obs_kwargs) -> np.ndarray:
        ...
        # Filter update x2.
        for _ in range(2):
            self.models = dyn.filter_update(self.models, self.zone_temp, self.u)
    ...
    def step_(self, action: np.ndarray, **obs_kwargs) -> tuple[np.ndarray, float, bool, dict]:
        ...
        self.model, self.zone_temp = dyn.dynamics(
            self.models, self.zone_temp, action, self.temp_oa, self.q_solar, self.q_int
        )
    ...
    def get_obs(self, **obs_kwargs) -> tuple[np.ndarray, dict]:
        ...
        self.state = OrderedDict({f"zone_temp_{k}": v for k, v in enumerate(self.zone_temp)})
        self.state.update({f"zone_upper_viol_{k}": v for k, v in enumerate(zone_upper_temp_viol)})
        self.state.update({f"zone_lower_viol_{k}": v for k, v in enumerate(zone_lower_temp_viol)})
        self.state.update(
            {
                ...
                "time_of_day": 1.0 * self.time_index / self.max_episode_steps,  # time,
...
class FiveZoneROMThermalEnergyEnv(FiveZoneROMEnv):
    ...
    def step_reward(self) -> tuple[float, dict]:
        ...
        comfort_error = [
            max(self.state[f"zone_upper_viol_{i}"], self.state[f"zone_lower_viol_{i}"], 0.0)
            for i in range(self.num_zones)
        ]
        comfort_reward = -(sum([x**2 for x in comfort_error]))

        reward = alpha * energy_consumption_reward * 0.5 + (1.0 - alpha) * comfort_reward

        meta = {"comfort_rew": comfort_reward, "energy_rew": energy_consumption_reward}
