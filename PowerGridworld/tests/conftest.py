...
def single_agent_episode_runner(env):
    ...
    results = [single_agent_single_episode_runner(env, kind) for kind in ["low", "high", "random"]]
...
def multi_agent_episode_runner(env):
    ...
    results = [multi_agent_single_episode_runner(env, p) for p in ["low", "high", "random"]]
...
def pf_config(opendss_config):
    return {"cls": OpenDSSSolver, "config": opendss_config}
...
def multicomponent_building_config():
    ...
    building = {
        ...
        "config": {
            ...
            "obs_config": {"zone_temp": (18, 34), "p_consumed": (-100, 100)},
        },
    }
    ...
    storage = {"name": "storage", "cls": EnergyStorageEnv, "config": {"rescale_spaces": False}}
...
@pytest.fixture(scope="function")
def pv_config():
    return {"name": "pv", "profile_csv": "pv_profile.csv", "scaling_factor": 10.0}


@pytest.fixture(scope="function")
def pv_array_config():
    return {"name": "pv", "profile_csv": "pv_profile.csv", "scaling_factor": 400.0}
