...
def plot_learning_curves(data_dir, exp_name):
    ...
    agent_num = int(len(logged_data["agrewards"]) / len(logged_data["rewards"]))

    logged_data["agrewards"] = np.array(logged_data["agrewards"])
    logged_data["agrewards"] = logged_data["agrewards"].reshape((-1, agent_num))

    fig, axes = plt.subplots(3, 2)

    for agent_idx in range(agent_num):
        axes[0, 0].plot(
            -1 * logged_data["agrewards"][:, agent_idx][:MAX_ITER],
    ...
    axes[0, 1].plot(-1 * np.array(logged_data["rewards"])[:MAX_ITER], color="k", alpha=0.8)
    axes[0, 1].grid()
